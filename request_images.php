<?php
  ...
  try {
    ...
    if((int)$resp->role >= $MIN_PERMISSION) {
      ...
      $sql = "SELECT * FROM products_images WHERE products_images.Pro_Id = '" . $id."';";
      ...
      if ($mysqli->affected_rows > 0) {
        ...
        while($row = $res->fetch_assoc()) {
          $data = new stdClass;
          $data->id = $row["ProImage_Id"];
          $data->image = $row["ProImage"];
          $data->title = $row["ProImage_Desc"];
          $dt->{$cont} = $data;
          $cont++;
        }
        $list->data = $dt;
        echo json_encode($list);
          ...
          $json = new stdClass;
          $json->status = "NO_IMAGES";
          $json->message = "No hay imagenes.";
          echo json_encode($json);
        }
